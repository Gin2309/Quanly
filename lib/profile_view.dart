...
class _Step2FormState extends State<Step2Form> {
  final streetClt = TextEditingController();
  ...
  Future<void> loadLocationData() async {
    try {
      String data = await rootBundle.loadString('asset/don_vi_hanh_chinh.json');
  ...
  @override
  Widget build(BuildContext context) {
    streetClt.text = widget.userInfo.address?.street ?? "";
    return Padding(
      padding: const EdgeInsets.only(bottom: 16),
      child: Form(
        key: widget.formKey,
                ...
                return TextFormField(
                  ...
                  validator: (value) {
                    if (widget.userInfo.address?.district == null ||
                        value!.isEmpty) {
                      return ("Vui lòng chọn một Quận / Huyện");
                    }
                    return null;
                  },
                );
